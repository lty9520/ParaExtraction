...
//求数组最大值
double
Tools::max_array(double *array, int n)//array 为int类型的数组的指针 ，n为数组元素个数
{
	double max = array[0];
	for (int i = 0; i < n; i++)
	{
...
//求数组最小值
double
Tools::min_array(double *array, int n)//array 为int类型的数组的指针 ，n为数组元素个数
{
	double min = array[0];
	for (int i = 0; i< n; i++)
	{
...
//计算点云密度（点云间平均间距）
double
Tools::computeCloudResolution(const pcl::PointCloud<PointXYZ>::ConstPtr &cloud)
{
	double res = 0.0;
	int n_points = 0;
	int nres;
...
//************************************
void Tools::boundaryEstimation(pcl::PointCloud<pcl::Boundary>& boundaries,
...
//************************************
void Tools::normalEstimation(pcl::PointCloud<pcl::Normal>::Ptr& cloud_normal,
